...
def calculate_returns(stock_data, market_data):
    ...
    stock_returns = stock_data[['Returns']].rename(columns={'Returns': 'Stock_Returns'})
    market_returns = market_data[['Returns']].rename(columns={'Returns': 'Market_Returns'})
...
def analyze_event(stock_data, merged, ed, window, ticker):
    ...
    summary_rows.append({
        ...
        'P-value':      round(p, 3),
        'Pre-Vol (%)':  round(pre_R.std() * 100, 2),
        'Post-Vol (%)': round(post_R.std() * 100, 2),
        'Pre-Volume':   round(stock_data.loc[pre_start:pre_end,  'Volume'].mean(), 0),
        'Post-Volume':  round(stock_data.loc[post_start:post_end,'Volume'].mean(), 0),
    })
...
def plot_data(all_event_data, window_label):
    ...
    window_size = 20
    for event_data in all_event_data:
        pre_vol  = event_data['pre_event_returns'].rolling(window=window_size).std()
        post_vol = event_data['post_event_returns'].rolling(window=window_size).std()
        axs2[1].plot(pre_vol.index,  pre_vol,  label=f'{event_data["ticker"]} Pre')
        axs2[1].plot(post_vol.index, post_vol, linestyle='--', label=f'{event_data["ticker"]} Post')
...
for window_label, window in windows.items():
    ...
    for event in events:
        ...
        if stock_data.empty or market_data.empty:
            print(f"Data missing for {stock_symbol} on {event_date}. Skipping.")
            continue
        ...
        if event_date not in stock_data.index:
            print(f"Event date {event_date} not in data for {stock_symbol}. Skipping.")
            continue

        event_window_data, days_offset, pre_event_returns, post_event_returns = analyze_event(
            stock_data, merged_data, event_date, window, stock_symbol
        )
